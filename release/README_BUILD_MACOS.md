...
Output (no databases):
- `release/PrimeRL_1.3.4_app_macos_arm64_nodb/dist/PrimeRL.app`

To include bundled databases:
```bash
./release/scripts/build_app_macos.sh --version 1.3.4 --clean --with-databases
```
...
./release/scripts/sign_app_macos.sh \
  --app release/PrimeRL_1.3.4_app_macos_arm64_nodb/dist/PrimeRL.app \
  --identity "Developer ID Application: Your Name (TEAMID)"
```

You can also build and sign in one call:
```bash
./release/scripts/build_app_macos.sh --version 1.3.4 --clean --sign
```

For distributable signing in one call:
```bash
./release/scripts/build_app_macos.sh \
  --version 1.3.4 --clean --sign \
